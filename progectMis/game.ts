...
function compare() {
    var scoreNow = (<HTMLInputElement> document.getElementById('CurrentScore')).innerHTML;
    //alert(scoreNow);
    ...
    for (var i = 0; flage; i++) { Div(); if (i == 100) flage = 0; }
    ...
    function Div() {
        ...
        if (square_pos[index] >= 1) {
            ...
            if (list.indexOf(currentLeft) == Number("-1")) {
                ...
                num = num + 1;
				 if(str == "green")
					elem.addEventListener("animationiteration", AnimationListener);
    ...
    function elementAction(str) {
		...
		a = a -10;
            for (var i = 1; i <= 11; i++) {
                ...
                element.style.width = (a - 10) + "px";
                element.style.height = (a - 10) + "px";
            }
